...
def write_output(output, output1, filename):
	...
	sheet1.col(10).width = 3333 * 4

	sheet2.col(0).width = 3333 * 2
	sheet2.col(1).width = 4200
	sheet2.col(2).width = 1666
	sheet2.col(3).width = 2666
	sheet2.col(4).width = 1666
	sheet2.col(5).width = 2666
	...
	for key_index in range(1, len(output1)):
		...
		for flavor_index in range(0, len(output1[key]["flavors"])):
			...
			# 规格
			sheet2.write(flavor_index + flavor_list_len + 2, 1, output1[key]["flavors"][flavor_index]["flavor"].decode("gbk"), style)
			# 数量
			sheet2.write(flavor_index + flavor_list_len + 2, 2, output1[key]["flavors"][flavor_index]["number"].decode("gbk"), style)
			# 进货斤数
			sheet2.write(flavor_index + flavor_list_len + 2, 3, "", style)
			# 进货价
			sheet2.write(flavor_index + flavor_list_len + 2, 4, "", style)
			# 对外售价
			sheet2.write(flavor_index + flavor_list_len + 2, 5, "", style)
			# 购货人
			sheet2.write(flavor_index + flavor_list_len + 2, 6, output1[key]["flavors"][flavor_index]["bussiness_name"].decode("gbk"), style)


		# 商品名
		sheet2.write_merge(flavor_list_len + 2, flavor_list_len + len(output1[key]["flavors"]) + 1, 0, 0, key.decode("gbk"), style)
		# 总规格
		sheet2.write_merge(flavor_list_len + 2, flavor_list_len + len(output1[key]["flavors"]) + 1, 7, 7, str(total_flavor) + flavor_unit, style)
		# 数量
		sheet2.write_merge(flavor_list_len + 2, flavor_list_len + len(output1[key]["flavors"]) + 1, 8, 8, str(total_number) + number_unit, style)
